...
#### Helper: descriptive statistics to adjust axes
...
### Read dw scatterplots files

```{r}

chart_results <- readRDS("data_output/scatterplot_chart_ids.RData")
```

### Export dw scatterplot PNGs

```{r}

library(magick)

export_dw_charts <- function(chart_ids, export_dir = "data_output/datawrapper/") {
  if (!dir.exists(export_dir)) dir.create(export_dir, recursive = TRUE)
  lapply(chart_ids, function(id) {
    # Desktop 400px
    png_400 <- dw_export_chart(
      chart_id      = id,
      type          = "png",
      unit          = "px",
      width         = 600,
      mode          = "rgb",
      plain         = FALSE, # titre header and footer in
      border_width  = 0,
      transparent   = FALSE,
      api_key       = "environment"
    )
    image_write(png_400, file.path(export_dir, paste0(id, "_400px.png")))
    # Mobile 320px
    png_320 <- dw_export_chart(
      chart_id      = id,
      type          = "png",
      unit          = "px",
      width         = 320,
    ...
    image_write(png_320, file.path(export_dir, paste0(id, "_320px.png")))
  })
}
...
png_export_test <- chart_results %>% filter(city_id == 5586) %>% pull(chart_id)

export_dw_charts(png_export_test)
```
...
### Charts for scrollytelling WITH annotations
...
-----------------------------------
# St-Gall
"https://app.datawrapper.de/edit/rffGE/visualize#refine"
# Geneva
"https://app.datawrapper.de/edit/8ZPgH/visualize#annotate"
# Lausanne
"https://app.datawrapper.de/edit/iPCow/visualize#refine"


sf_grouped %>% filter(bfs_nummer == 6621) %>% 
  select("ssep3", "no2", "ssep_no2_label") %>% 
  dw_data_to_chart("8ZPgH")

sf_grouped %>% filter(bfs_nummer == 5586) %>% 
  select("ssep3", "no2", "ssep_no2_label") %>% 
  dw_data_to_chart("iPCow")


# -------------- NDVI --------------
------------------------------------
  # Lausanne 
"https://app.datawrapper.de/edit/HfB0J/visualize#refine"

# Genève
"https://app.datawrapper.de/edit/qRJ8l/visualize#refine"
...
------------------------------------
  Lausanne
"https://app.datawrapper.de/edit/MJu0Z/visualize#refine"
...
# # Export PNGs
# export_dw_charts(scatterplots_scrolly_ids)
...
export_for_mapbox <- function(hotspots_sf, filename) {
  if(nrow(hotspots_sf) > 0) {
    ...
    hotspots_wgs84 <- hotspots_wgs84 %>% 
      select(city, category, indicator)
...
# Fonction pour nettoyer les hotspots avec mapshaper dissolve2
clean_hotspots_with_mapshaper <- function(hotspots_sf, 
                                         indicator_name,
                                         output_dir = "data_output/mapbox",
                                         gap_fill_area = "200m2",
                                         sliver_control = 1) {
  ...
  if(nrow(hotspots_sf) == 0) {
    cat("❌ Aucun hotspot pour", indicator_name, "\n")
    return(NULL)
  }
  ...
  temp_output <- file.path(output_dir, paste0("temp_", indicator_name, "_cleaned.geojson"))
  ...
  # Commande mapshaper avec dissolve2
  ...
  cat("🔧 Nettoyage", indicator_name, "avec dissolve2...\n")
  cat("Commande:", mapshaper_cmd, "\n")
  ...
  if(file.exists(final_output)) {
    ...
    cat("✅", indicator_name, ":", nrow(hotspots_wgs84), "→", nrow(cleaned_sf), "polygones\n")
    
    # Statistiques rapides
    if("n_households" %in% names(hotspots_wgs84)) {
      cat("   Ménages avant:", sum(hotspots_wgs84$n_households, na.rm = TRUE), "\n")
    }
    cat("   Fichier:", file.size(final_output), "bytes\n\n")
    ...
    cat("❌ Erreur lors du nettoyage de", indicator_name, "\n")
    return(NULL)
  }
}

# Appliquer le nettoyage aux 3 indicateurs
output_dir <- "data_output/mapbox"

# Nettoyer chaque indicateur
...
for(f in final_files) {
  ...
  cat("📁", filename, "-", size_kb, "KB -", n_features, "polygones\n")
}
```
