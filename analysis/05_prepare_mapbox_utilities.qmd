...
-   `data_output/mapbox_vectors/*.geojson` - Classified environmental layers (noise, NO₂, NDVI)
...
```{r}
cat("🔶 Creating anonymized SES hexagons...\n")
...
cat("✅ SES hexagons exported to data_output/mapbox/sep_hex.geojson\n")
...
```{r}
cat("🗺️ Creating clean municipal boundaries...\n")
...
cat("✅ Clean city boundaries exported to data_output/mapbox/cities.geojson\n")
...
```{r}
cat("📍 Calculating city centroids...\n")
...
cat("✅ City centroids exported to data_output/city_centroids_mapbox.csv\n\n")
```
...
cat("   NO₂: CRS", crs(rast_no2, describe = TRUE)$name, "\n") 
...
# NO₂: 5 μg/m³ increments from 0 to 40+
...
cat("📋 Classification thresholds defined:\n")
cat("   Noise:", length(labels_noise), "classes\n")
cat("   NO₂:", length(labels_no2), "classes\n")
...
cat("🔄 Processing raster vectorization...\n")
...
for (i in 1:nrow(sf_cities_10)) {
  ...
  if (!is.null(sf_noise) && nrow(sf_noise) > 0) {
    ...
    cat("    ✅ Noise:", basename(filename), "(", size_kb, "KB)\n")
  }
  ...
  if (!is.null(sf_no2) && nrow(sf_no2) > 0) {
    ...
    size_kb <- round(file.size(filename) / 1024, 1)
    cat("    ✅ NO₂:", basename(filename), "(", size_kb, "KB)\n")
  }
  ...
  if (!is.null(sf_ndvi) && nrow(sf_ndvi) > 0) {
    ...
    size_kb <- round(file.size(filename) / 1024, 1)
    cat("    ✅ NDVI:", basename(filename), "(", size_kb, "KB)\n")
  }
}

cat("\n✅ All raster vectorization complete!\n\n")
```
  ...
  for (file in c(hex_files, boundary_files, centroid_files)) {
    if (file.exists(file)) {
      ...
      if (grepl("\\.geojson$", file)) {
        ...
        cat("📁", basename(file), "-", size_kb, "KB -", n_features, "features\n")
      } else {
  ...
  if (length(vector_files) > 0) {
    for (file in vector_files) {
      if (file.exists(file)) {
        ...
        total_size_kb <- total_size_kb + size_kb
        n_features <- nrow(st_read(file, quiet = TRUE))
        cat("📁", basename(file), "-", size_kb, "KB -", n_features, "polygons\n")
      }
    }
  } else {
    cat("❌ No vector files found in data_output/mapbox_vectors/\n")
  }
...
cat("\n🌍 All assets ready for web integration!\n")
```
...
cat("\n📖 USAGE INSTRUCTIONS\n")
cat("=====================\n")
cat("1. 🔶 sep_hex.geojson - Add as Mapbox layer for SES patterns\n")
cat("2. 🗺️ cities.geojson - Use as boundary overlay\n") 
cat("3. 📍 city_centroids_mapbox.csv - Use for map navigation/labeling\n")
cat("4. 📊 mapbox_vectors/*.geojson - Classified environmental layers:\n")
...
cat("Ready for Mapbox Studio integration! 🚀\n")
```
